...
async function performBackgroundSync(): Promise<void> {
  try {
    const data: DealflowStorage = await chrome.storage.sync.get(["apiToken", "serverUrl"]);
...
async function handleLeadDataSync(payload: any): Promise<any> {
  const data: DealflowStorage = await chrome.storage.sync.get(["apiToken", "serverUrl"]);
  const serverUrl = data.serverUrl || DEFAULT_SERVER_URL;
