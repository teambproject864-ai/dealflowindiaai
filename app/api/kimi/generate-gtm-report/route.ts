...
export async function POST(request: NextRequest) {
  ...
  const requestId = `req-gtm-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
  ...
  try {
    ...
    // Extract headers for mTLS check if internal call
    const headersRecord: Record<string, string> = {};
    request.headers.forEach((val, key) => {
      headersRecord[key] = val;
    });

    const reportOutput = await kimiContentOrchestrator.generateGTMReport(body, headersRecord);
    ...
    await logModelInvocation({
      ...
      tokensOut: Math.ceil(JSON.stringify(reportOutput).length / 4),
      latency: latencyMs,
      gpuId: "Kimi Cluster",
    ...
    console.error("[api/kimi/generate-gtm-report] Error:", error);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: "Validation failed for GTM report parameters",
    ...
    const unifiedErr = handleKimiError(error, { requestId, endpoint: "/api/kimi/generate-gtm-report" });
