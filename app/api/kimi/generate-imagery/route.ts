...
export async function POST(request: NextRequest) {
  ...
  const requestId = `req-img-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
  ...
  try {
    ...
    const imageryOutput = await kimiContentOrchestrator.generateImagerySpecs(body, headersRecord);
    ...
    await logModelInvocation({
      ...
      tokensOut: Math.ceil(JSON.stringify(imageryOutput).length / 4),
      latency: latencyMs,
      gpuId: "Kimi Cluster",
    ...
    console.error("[api/kimi/generate-imagery] Error:", error);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: "Validation failed for imagery parameters",
    ...
    const unifiedErr = handleKimiError(error, { requestId, endpoint: "/api/kimi/generate-imagery" });
